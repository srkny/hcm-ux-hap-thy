/*global location window setTimeout setInterval clearTimeout clearInterval console document _*/
sap.ui.define(
  [
    "hcm/ux/hapv4/controller/BaseController",
    "sap/ui/model/json/JSONModel",
    "sap/ui/core/routing/History",
    "hcm/ux/hapv4/model/formatter",
    "sap/m/MessageToast",
    "sap/m/MessageBox",
  ...
  function (
    ...
    return BaseController.extend("hcm.ux.hapv4.controller.FormDetails", {
      formatter: formatter,
      hasChanges: false,
      _aFormUIElements: [],
      ...
      _prepareSideBarData: function (sAppraisalId) {
        ...
        if (sMainAppraisalId === sAppraisalId) {
          ...
          oSideBarData.appeeInfo.ImageSource =
            "/sap/opu/odata/sap/ZHCM_UX_HAP_TKAO_SRV_01/EmployeeInfoSet('" +
            oAppraisee.Id +
            "')/$value";
          ...
          oSideBarData.statusInfo.push(
            {
              Label: this.getText("labelApStatus"),
              Value: oStatus.ApStatusName,
            },
            {
              Label: this.getText("labelApStatusSub"),
              Value: oStatus.ApStatusSubName,
            },
            {
              Label: this.getText("labelApStartDate"),
              Value: this._formatDate(oDates.ApStartDate),
            },
            {
              Label: this.getText("labelApEndDate"),
              ...
              Data: [
                {
                  ...
                  Value: this.getText("SUMMARY_HEADER_TITLE"), //SUMMARY_HEADER_TITLE
                  Type: "HeaderEmpty",
                  ColumnIndex: sColIndex,
                  RowIndex: sRowIndex,
                },
      ...
      _getDocumentDetail: function () {
        ...
        var sExpand =
          ...
          "FeAlreadyChosen,FeFlatAvailable,FeSelectableOtype,FeStrucAvailable," +
          "FeBodyElementsAdd,ReturnOp,FormQuestions,FormAnswers,Competencies," +
          "Objectives,FormParameters,ResultTable,DevPlanActivities,Intros,StatusNotes";
          ...
          if (oBodyCells[sRowIid].hasOwnProperty(oColumn.ColumnIid)) {
            ...
            if (
              ...
              if (
                ...
                oFC.setLayoutData(
                  new sap.ui.layout.GridData({
                    span: "XL10 L10 M12 S12",
                  })
                );
              } else if (
                oCell.ColumnIid ===
                  oThis._getColumnIid(oCell.AppraisalId, "ZTK4") &&
                oCell.LayoutType === "R"
              ) {
                oFC.setLayoutData(
                  new sap.ui.layout.GridData({
        ...
        if (oCell.RowIid !== "0000" && oCell.ColumnIid !== "0000") {
          if (
            ...
            if (
              ...
              oTFB.addItem(oEl);
              var oHTML = new sap.ui.core.HTML({
                content: "<span hidden class='hapTooltipIcon'>&nbsp;</span>",
              });
              oTFB.addItem(oHTML);
              ...
              // 095391 - TTAS için yazılmış kapatıldı
              /*	oTFB.addItem(new sap.ui.core.HTML({
                                                content: "<span class='hapTooltipText'>Lütfen 5’li skalada bir değer giriniz. Gireceğiniz değer tam ya da ondalıklı olabilir</span>"
                                            }));*/
              oTFB.addItem(
                new sap.ui.core.HTML({
                  content: "<span class='hapTooltipText'> </span>",
                })
              );
              oFB.addItem(oTFB);
            } else {
        ...
        if (aValueDesc.length > 0) {
          if (!oThis._oValDescPopover) {
            oThis._oValDescPopover = sap.ui.xmlfragment(
              "hcm.ux.hapv4.fragment.ValueDescription",
              this
            );
      ...
      _handleCallSurvey: function (
        ...
        if (!this._oSurveyDialog) {
          ...
          this._oSurveyDialog = sap.ui.xmlfragment(
            "hcm.ux.hapv4.fragment.ElementSurvey",
            this
          );
      ...
      _handleActionButtonPressed: function (oEvent) {
        ...
        switch (oButton.data("ButtonId")) {
          ...
          case "START_DEV_PLAN":
            ...
            break;
          case "STAT_LOG":
            this._handleShowStatNotes(sAppraisalId);
            break;
          case "SHOW_INTRO":
      ...
      _handleShowStatNotes: function (sAppraisalId) {
        var oThis = this;
        var oViewModel = this.getModel("formDetailsModel");
        var aStatNotes = oViewModel.getProperty(
          "/formData/" + sAppraisalId + "/StatusNotes"
        );
        var sNotes = "";

        $.each(aStatNotes, function (i, oNote) {
          sNotes = sNotes + oNote.StringNote + "\n";
        });
          ...
          switch (oReturn.Type) {
            case "S":
              MessageBox.show(oReturn.Message, {
                ...
                title: oThis.getResourceBundle().getText("INFO_DIALOG_TITLE"),
      ...
      _handleListAttachment: function (oEvent) {
        ...
        if (!this._oListAttachmentDialog) {
          ...
          this._oListAttachmentDialog = sap.ui.xmlfragment(
            "hcm.ux.hapv4.fragment.AttachmentList",
            this
          );
          // connect dialog to view (models, lifecycle)
      ...
      _getChildrenCount: function (sAppraisalId, sRowIid) {
        ...
        $.each(aBodyElements, function (sIndex, oBodyElement) {
          if (
            oBodyElement.Parent === sRowIid &&
            oBodyElement.ForeignType !== ""
          ) {
            sChildrenCount++;
          }
        });
      ...
      _enhanceDocumentFromCatalog: function (
        aSelectedObjects,
        aAlreadySelectedObjects
      ) {
        var oViewModel = this.getModel("formDetailsModel");
        ...
        if (aSelectedObjects.length > 1) {
          MessageBox.warning(
            this.getResourceBundle().getText("oneChildSelectionReached", [
        ...
        $.each(aSelectedObjects, function (sIndex, oSelectedObject) {
          var oOneAlreadySelectedChild = _.find(aAlreadySelectedObjects, [
            "PupObjid",
            oSelectedObject.PupObjid,
          ]);
          if (oOneAlreadySelectedChild) {
            oReturn = false;
          }
        });
        ...
        if (!oReturn) {
          MessageBox.warning(
            oThis.getResourceBundle().getText("oneChildAlreadySelectionReached")
          );
          return;
        }

        // if (aSelectedObjects.length > sSpace) {
        // 	MessageBox.warning(this.getResourceBundle()
        // 		.getText("maxChildSelectionReached", [sSpace, aSelectedObjects.length]));
        // 	return;
        // }
        // }
      ...
      expandFirstLevel: function () {
        var oTreeTable = sap.ui.getCore().byId("TreeTableCatalog");
      ...
      _getFeedBack: function (
        ...
        var oFeedBackDialog = new sap.m.Dialog({
          ...
          endButton: new sap.m.Button({
            text: oThis.getResourceBundle().getText("labelCancel"),
            press: function () {
              oFeedBackDialog.close();
            },
          }),
      ...
      _openAddNewElementFreeDialog: function () {
        ...
        if (!this._oAddNewElementFreeDialog) {
          ...
          this._oAddNewElementFreeDialog = sap.ui.xmlfragment(
            "hcm.ux.hapv4.fragment.AddNewElementFree",
            this
          );
          // connect dialog to view (models, lifecycle)
      ...
      _openAddNewElementFreeFormDialog: function (
        ...
        if (!this._oAddNewElementFreeFormDialog) {
          ...
          this._oAddNewElementFreeFormDialog = sap.ui.xmlfragment(
            "hcm.ux.hapv4.fragment.AddNewElementFreeForm",
            this
          );
          //escape handler
      ...
      _openAddNewElementObjectiveDialog: function (sAppraisalId) {
        ...
        if (!this._oAddNewElementObjectiveDialog) {
          ...
          this._oAddNewElementObjectiveDialog = sap.ui.xmlfragment(
            "hcm.ux.hapv4.fragment.AddNewElementObjective",
            this
          );
          //escape handler
      ...
      _openAddNewElementCatalogDialog: function () {
        ...
        if (!this._oAddNewElementCatalogDialog) {
          ...
          this._oAddNewElementCatalogDialog = sap.ui.xmlfragment(
            "hcm.ux.hapv4.fragment.AddNewElementCatalog",
            this
          );
          // connect dialog to view (models, lifecycle)
      ...
      onApplyAddElementCatalog: function (oEvent) {
                ...
                if (
                  ...
                  if (!oChild.AlreadySelected && oChild.Selected) {
                    var oSelectedObject = {
                      Otype: oChild.Otype,
                      Objid: oChild.Objid,
                      PupObjid: oChild.PupObjid,
                    };
                    aSelectedObjects.push(oSelectedObject);
                  } else if (oChild.AlreadySelected) {
                    var oAlreadySelectedObject = {
                      Otype: oChild.Otype,
                      Objid: oChild.Objid,
                      PupObjid: oChild.PupObjid,
                    };
                    aAlreadySelectedObjects.push(oAlreadySelectedObject);
                  }
                }
        ...
        if (aSelectedObjects.length > 0) {
          this._enhanceDocumentFromCatalog(
            aSelectedObjects,
            aAlreadySelectedObjects
          );
        } else {
      ...
      _openUploadAttachmentDialog: function () {
        ...
        if (!this._oUploadAttachmentDialog) {
          ...
          this._oUploadAttachmentDialog = sap.ui.xmlfragment(
            "hcm.ux.hapv4.fragment.UploadAttachments",
            this
          );
          // connect dialog to view (models, lifecycle)
      ...
      _showDevTrainings: function (oEvent) {
        if (!this._oDevTrainingsDialog) {
          ...
          this._oDevTrainingsDialog = sap.ui.xmlfragment(
            "hcm.ux.hapv4.fragment.DevelopmentTrainings",
            this
          );
          // connect dialog to view (models, lifecycle)
