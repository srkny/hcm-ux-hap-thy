...
sap.ui.define([
	...
	return BaseController.extend("hcm.ux.hapv4.controller.FormList", {
		 ...
		 * @private
		 */
		_checkUsage: function(sUsage) {
			var oModel = this.getModel();
			var oViewModel = this.getModel("formDetailsModel");
			var oThis = this;

			oModel.callFunction("/CheckUsage", {
				method: "GET",
				success: function(oData, oResponse) {
					debugger;
					if (oData.Isusage === false) {
						sUsage = false;
						MessageBox.warning("Uygulamaya bu dönemde erişim yetkiniz bulunmuyor. Erişim için IK ile iletişime geçiniz.", {
							actions: [MessageBox.Action.CLOSE],
							onClose: function(sAction) {
								oThis.onNavHome();
							}.bind(this)
